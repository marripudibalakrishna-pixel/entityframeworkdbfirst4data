...
namespace entityframeworkdbfirst4data.Controllers
{
    ...
    public class EmployeeController : ControllerBase
    {
        ...
        public async Task<IActionResult> delete(int empid)
        {
            ...
            try
            {
                var empdata = await _employeeservice.deletemployee(empid);
                if (empdata == null)
        ...
        public async Task<IActionResult> GetEmployees()
        {
            try
            {
                var empdata = await _employeeservice.GetAllemployees();
                if (empdata == null)
                {
                    return StatusCode(StatusCodes.Status404NotFound, "empdata not  found");
                }
        ...
        public async Task<IActionResult> Get(int empid)
        {
            ...
            try
            {
                var empdata = await _employeeservice.GetEmployee(empid);
                return StatusCode(StatusCodes.Status200OK, empdata);
            }
            catch (Exception ex)
            {
